...
public class Track {
    ...
    public interface OnInfoListener
    {
        ...
        boolean onInfo(Track mp, int what, int extra);
    }

    public interface OnErrorListener
    {

        boolean onError(Track mp, int what, int extra);
    }
    ...
    public void pause() {
        ...
        case STATE_PAUSED:
            ...
            MLogUtil.d(TAG_TRACK, "State changed to STATE_PAUSED");
            break;
        default:
            error();
        }
    ...
    public void prepare() {
        ...
        case STATE_STOPPED:
            ...
            MLogUtil.d(TAG_TRACK, "State changed to STATE_PREPARED");
    ...
    public void prepareAsync() {
        ...
        case STATE_STOPPED:
            ...
            MLogUtil.d(TAG_TRACK, "State changed to STATE_PREPARING");
            ...
            Thread t = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if(onPreparedListener != null){
                        onPreparedListener.onPrepared(Track.this);
                    }
    ...
    public void stop() {
        ...
        case STATE_PLAYBACK_COMPLETED:
            ...
            MLogUtil.d(TAG_TRACK, "State changed to STATE_STOPPED");
            mContinue = false;
            mTrack.pause();
            mTrack.flush();
            break;
    ...
    public void start() {
        ...
        case STATE_PLAYBACK_COMPLETED:
            ...
            MLogUtil.d(TAG_TRACK, "State changed to STATE_STARTED");
            mContinue = true;
            mTrack.play();
            decode();
        case STATE_STARTED:
        ...
        default:
            ...
            MLogUtil.d(TAG_TRACK, "State changed to STATE_ERROR in start");
            if (mTrack != null) {
                error();
            } else {
                MLogUtil.d("start",
    ...
    public void reset() {
        ...
        mCurrentState = STATE_IDLE;
        MLogUtil.d(TAG_TRACK, "State changed to STATE_IDLE");
        mLock.unlock();
    }
            ...
            Thread t = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if(onSeekCompleteListener != null){
                        onSeekCompleteListener.onSeekComplete(Track.this);
                    }
                    mLock.unlock();
                }
            });
    ...
    public void setDataSource(String path) {
        ...
        case STATE_IDLE:
            ...
            MLogUtil.d(TAG_TRACK, "Moving state to STATE_INITIALIZED");
            break;
        default:
            error();
        }
    ...
    public void initStream() throws IOException {
        ...
        if (mPath != null) {
            ...
            mExtractor.setDataSource(mFileDescriptor, offset, length);
        }else {
    ...
    public void decode() {
        mDecoderThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (!sawInputEOS && !sawOutputEOS && mContinue) {
                    ...
                    do {
                        ...
                        if (res >= 0) {
                            ...
                            MLogUtil.d("PCM", "Output buffers changed");
                            ...
                            final MediaFormat oformat = mCodec
                                    .getOutputFormat();
                            MLogUtil.d("PCM", "Output format has changed to"
                                    + oformat);
                            initDevice(
                ...
                MLogUtil.d(TAG_TRACK,
                        "Decoding loop exited. Stopping codec and track");
                MLogUtil.d(TAG_TRACK, "Duration: " + (int) (mDuration / 1000));
                MLogUtil.d(TAG_TRACK,
                        "Current position: "
                                + (int) (mExtractor.getSampleTime() / 1000));
                mCodec.stop();
                mTrack.stop();
                MLogUtil.d(TAG_TRACK, "Stopped codec and track");
                MLogUtil.d(TAG_TRACK,
                        "Current position: "
                                + (int) (mExtractor.getSampleTime() / 1000));
                mIsDecoding = false;
                if (mContinue && (sawInputEOS || sawOutputEOS)) {
                    ...
                    Thread t = new Thread(new Runnable() {
                        ...
                        public void run() {
                            if(onCompletionListener != null){
                                onCompletionListener.onCompletion(Track.this);
                            }
                        }
                    });
                    t.setDaemon(true);
                    ...
                    MLogUtil.d(TAG_TRACK, "sawInputEOS: " + sawInputEOS);
                    MLogUtil.d(TAG_TRACK, "sawOutputEOS: " + sawOutputEOS);
                }
                synchronized (mDecoderLock) {
